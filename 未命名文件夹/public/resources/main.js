...
controls.init();
  ...
  function loadData(order=1) {
      ...
      wf.onload(function() { 
          ...
          var selectedDate = selectData(data, time, frequency, timeRate, audio_bg.height /2);
          ...
          initBgCanvasHeight = audio_bg.height;
      });
  }
  			...
  			polygons.forEach(function(polygon){
  				...
  				if(bg_context.isPointInPath(loc.x, loc.y)){
  					...
  					if(polygon.isPointInLeft(loc)){
  						audio_bg.style.cursor = 'crosshair';
	  		...
	  		if(rectDrag.isInLeft){
	  			...
	  			if(rectDrag.x2 + deltaX - rectDrag.x1 > 10){
	  				if(polygons[index + 1]){
	  					...
	  					if(rectDrag.x2 + deltaX <= audio_bg.width){
	  						rectDrag.changeX2(deltaX);
	  						rectDrag.changeScaleX2(deltaX, initRate);
	  					}
	  				}
	  		  ...
	  		  if(polygons.length != 0){
					...
					if(position === -1){  // 最左边
						if(loc.x >= coordOffsetX && loc.x < polygons[0].x1){
							...
							if(guidewires){
								util.drawGuidewires(audio_bg, loc.x, loc.y);
							}	
						}
					}else if(position === - 2){  //最右边
						if(loc.x > polygons[polygons.length - 1].x2 && loc.x < audio_bg.width){
							util.restoreDrawingSurface( audio_bg ,surfaceData);
							bgDrawRubberband(loc);
