window.onload = function(){
  ...
  var audio = select('audio'),
      ...
      audio_img = select('audio_img'),

  	  // canvas context
  	  rect_context = audio_rect.getContext('2d'),
      bg_context = audio_bg.getContext('2d'),
      img_context = audio_img.getContext('2d'),
	    // a b and ab button
      ...
      scroll_button = select('scroll_button'),

      img_wrap = select('img_wrap');
  ...
  var view = {
    ...
    renderAudioRect: function(){
      rect_context.clearRect(0, 0, audio_rect.width, audio_rect.height);
      drawRects();
    },
...
controls.init();
	    ...
	    if(loc.x < coordOffsetX) return;
			...
			if(editing){
				polygons.forEach(function(polygon){
					polygon.createPath();
					// 判断点是否在矩形内 
					if(rect_context.isPointInPath(loc.x, loc.y)){
  ...
  function loadData(order=1) {
      ...
      wf.onload(function() { 
          ...
          // 重置状态
          clearInterval(currentTimeFlag);  
          rect_context.rect(0, 0, audio_rect.width, audio_rect.height);
          if(polygons.length){
            polygons = [];
            view.renderBarAfterDel();
          ...
          // img层canvas宽度
          audio_img.width = Math.round(time + 1) * timeRate > 800 ? Math.round(time + 1) * timeRate : 800; 

          

          middle_audio_img = audio_bg;
         
          img_context.drawImage(middle_audio_img, 0, 0);
          
          // img_context.beginPath();
          // img_context.rect(20.5 + 50,50,100,60);
          // img_context.strokeStyle = 'red';
          // img_context.stroke();


          // util.restoreDrawingSurface(audio_img, beforeVernierData);
          // img_context.drawImage(beforeVernierData, 0 , 0)
      });
  }
  var scrollX = 100
  scroll_button.addEventListener('click', function(e){
    var len = 10;
    scrollX = scrollX + len;
    img_wrap.scrollLeft = scrollX;
  })
  audio_img.addEventListener('click', function(e){
    var loc = util.windowToCanvas(audio_img, e.clientX, e.clientY);

    // img_wrap.scrollLeft = loc.x;

    console.log(loc);
  })

  // audio_img.addEventListener('')
  audio_img.addEventListener('mousewheel', imgMousewheel,false)
  var initRate = 0;
  // 变量
  // image 初始宽度
  var height;
  var width;
  // canvas初始宽度
  var initCanvasWidth = audio_img.width;
  var initCanvasHeight = audio_img.height;

  function imgMousewheel(e){
      e.stopPropagation();
      e.preventDefault(); 
      ...
      // 1 + initRate 滚动后进行计算的比例
      // 

      var scaleWidth = middle_audio_img.width * (1 + initRate);
      ...
      var scaleHeight = middle_audio_img.height;

      if(scaleWidth > initCanvasWidth){
        audio_img.width = scaleWidth;
      }else if(scaleWidth <= initCanvasWidth){
        audio_img.width = initCanvasWidth;
      }

      console.log('scaleWidth:' + scaleWidth);

      img_context.clearRect(0, 0, audio_img.width, audio_img.height);

      // img_context.drawImage(middle_audio_img, 0, 0, scaleWidth, scaleHeight);
      // 背景网格
      var gridWidth = 70;
      var griHeight = 20;
      var drawBcakgrounGrid = new DrawBcakgrounGrid(audio_img, gridWidth * initRate, griHeight * initRate);
      drawBcakgrounGrid.paint();        
      // 音频信号
      // console.log('原点Y');
      // console.log(coordOffsetY)
      // var drawLine = new DrawLine(audio_img, changedData, coordOffsetX, coordOffsetY, initRate);
      // drawLine.paint(); 
      // // 坐标轴
      // var drawCoord = new DrawCoord(audio_img, coordOffsetX, coordOffsetY , timeRate * initRate);
      // drawCoord.paint();
      // img_context.drawImage(middle_audio_img, 0, 0);
      // img_context.beginPath();
      // img_context.rect(20.5 + 50 * initRate,50,100 * initRate,60);
      // img_context.strokeStyle = 'red';
      // img_context.stroke();
      // console.log('放大比例：');
      // console.log(initRate);
      // console.log(timeRate * initRate)
      // console.log(loc);
